...
from LandedProgram.msgdev import MsgDevice, PeriodTimer

from LandedProgram.PID import PID
from LandedProgram.Simulator import trimaran_model, decode_state, encode_state, apply_noise
from LandedProgram.utils import plot, POSX, POSY, YAW, SPD, YAWSPD, plot_traj
...
def main():
    ...
    # 计算频率
    interval = 0.1
    pid_spd = PID(kp=3000.0, ki=100.0, kd=0, minout=0, maxout=2000, sampleTime=0.1) # TODO sampleTime
    ...
    try:
        while True:
            with t:
                ...
                # 追踪目标
                if USE_Fake_Goal:
                    goal[0] = fakedata3[i + 1000, 0]
                    goal[1] = fakedata3[i + 1000, 1]
                elif USE_Real_Goal:
                ...
                if USE_Fake_Obstacle:
                    ...
                    ob[1, :] = fakedata[i, :]
                    ob[2, :] = fakedata2[i + 300, :]
                    ob[3, :] = fakedata2[i + 1300, :]
                    ob[4, :] = fakedata3[i, :]
                    obstacle = ob
                elif USE_Real_Obstacle: #POSX, POSY, SPEED, Track
                    obstacle = [[target_state[POSX], target_state[POSY], target_state[4], target_state[7]]]
